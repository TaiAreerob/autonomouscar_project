...
//previous callback is calculate diff from quaternion(angle), but this callback integral angular velocity
//this function is still capable of using imu only, and using imu with odometry
{
  imu = *input;
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber imu_sub = nh.subscribe(_imu_topic.c_str(), _queue_size * 10, imu_callback); //tw
